...
O backend central da Fase 0 está implementado: perfil e hobbies, catálogo oficial, sessões paginadas, atributos dinâmicos em JSONB, equipamentos, backlog/Kanban, streak, localização por `place_id`, fotos por presigned upload, processamento WebP/thumbnail sem EXIF e documentação OpenAPI.
...
- Chaves temporárias são namespaced pelo UID autenticado; o backend rejeita key alheia, repetida ou acima do limite de dez fotos.
...
| `R2_BUCKET` | não | bucket isolado por ambiente | fotos/backup |
...
1. Criar buckets separados para dev e prod, ou isolamento equivalente.
2. Criar credencial S3 com acesso somente ao bucket/prefixos necessários.
3. Preencher endpoint, bucket, access key e secret no ambiente.
4. Validar presigned `PUT` com `Content-Type` e `Content-Length` assinados.
5. Validar o ciclo completo: upload temporário, associação à sessão, processamento, leitura via estratégia privada/CDN e remoção após excluir foto/sessão.
6. Configurar lifecycle para uploads temporários abandonados, como proteção adicional.
